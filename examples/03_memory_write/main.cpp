...
uint8_t read_memory_block(uint8_t zone_id, uint8_t block_id, uint8_t *data,
  ...
  // Mode: 0 -> 메모리 직접 읽기
  // Param1: Address (Zone/Block 주소)
  // Param2: Length (읽을 길이)
  uint8_t ret = aes132m_execute(
      AES132_BLOCK_READ, // [OpCode] BlockRead (0x02): 블록 읽기 명령
      0,                 // [Mode] 0: 메모리 직접 읽기
      param1,            // [Param1] Address: 읽을 Zone/Block 주소
      param2,            // [Param2] Length: 읽을 데이터 길이
...
void setup(void) {
  ...
  uint8_t test_data1[16] = {0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20,
                            0x41, 0x45, 0x53, 0x31, 0x33, 0x32,
                            0x21, 0x00, 0x00, 0x00}; // "Hello AES132!"
