...
uint8_t configure_key_slot(uint8_t key_id) {
  ...
  uint8_t config_data[4] = {0x1C, 0x00, 0x00,
                            0x00}; // 4바이트 단위 쓰기 권장 (Word Align)
...
void setup(void) {
  ...
  // Mode: 0 -> 메모리 직접 읽기
  // Param1: Address (0xF020) -> LockConfig 및 KeyConfig 시작 주소
  // Param2: Length (4)
  ret_read = aes132m_execute(
      AES132_BLOCK_READ, // [OpCode] BlockRead (0x02): 블록 읽기 명령
      0,                 // [Mode] 0: 메모리 직접 읽기
      0xF020,            // [Param1] Address: KeyConfig[0] 시작 주소
      4,                 // [Param2] Length: 읽을 데이터 길이
      0,                 // [Data1 Length] 0: 입력 데이터 없음
      NULL,              // [Data1 Pointer] NULL: 입력 데이터 없음
      0,                 // [Data2 Length] 0: 입력 데이터 없음
  ...
  if (ret_read == AES132_DEVICE_RETCODE_SUCCESS) {
    ...
    if (count >= 4 + 2) {
      ...
      // Analysis of Byte 2 (KeyConfig[1] or [2] depending on
      // endianness/datasheet, usually Byte 1 index) But let's print raw bytes
      // first.
    }
  } else {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    // [AES132 Datasheet 8.4 BlockRead Command]
    // OpCode: 0x02 (AES132_BLOCK_READ)
    // Mode: 0
    // Param1: Address (KeyConfig Address)
    // Param2: Length (4 bytes)
    ret_read = aes132m_execute(
        AES132_BLOCK_READ, // [OpCode] BlockRead (0x02): 블록 읽기 명령
        0,                 // [Mode] 0: 메모리 직접 읽기
        addr,              // [Param1] Address: KeyConfig 주소
        4,                 // [Param2] Length: 읽을 데이터 길이
        0,                 // [Data1 Length] 0: 입력 데이터 없음
        NULL,              // [Data1 Pointer] NULL: 입력 데이터 없음
        0,                 // [Data2 Length] 0: 입력 데이터 없음
  ...
  if (ret != AES132_DEVICE_RETCODE_SUCCESS) {
    Serial.println("Note: Configuration might fail if device is Locked. "
                   "Proceeding to Key Load...");
  }
  ...
  uint8_t my_key[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
