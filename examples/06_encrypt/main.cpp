/**
 * @file main.cpp
 * @brief Example 06: AES Encryption (Fixed for AES-CCM)
 *
 * Demonstrates AES-128 encryption using ATAES132A.
 * CRITICAL UPDATE: AES-CCM mode returns both MAC and Ciphertext.
...
bool encrypt_block(uint8_t key_id, const uint8_t* plaintext, uint8_t* out_mac, uint8_t* out_ciphertext) {
    ...
    if (ret == AES132_DEVICE_RETCODE_SUCCESS) {
        ...
        // Expected Length: 1 (Count) + 1 (Status) + 16 (MAC) + 16 (Ciphertext) + 2 (CRC) = 36 bytes
        if (count >= 36) {
            // Correct indexing:
            // Index 0: Count
            // Index 1: Status
...
void setup() {
    ...
    if (encrypt_block(KEY_SLOT_ID, plaintext, mac, ciphertext)) {
        ...
        Serial.print("MAC:        "); print_hex("", mac, 16);
